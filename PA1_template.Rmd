...
The mean of steps taken per day is `r round(mean_steps,0)`.
The median of taken steps is `r round(median_steps,0)`.
...
The interval `r max_steps[1]` contains the maximum number of steps which is `r round(max_steps[2])` on average across all the days.
...
The mean of steps taken per day is `r round(mean_noNAs,0)`.
The median of taken steps is `r round(median_noNAs,0)`. 
